...
def get_reservable_info(hotel_names, urls):
    ...
    last_reservation_date = (
        (datetime.datetime.now() + datetime.timedelta(days=44))
        .date()
        .strftime("%Y月%m月%d日")
    )
...
def convert_reservable_info_into_text(
    last_reservation_date: str, reservable_info: dict
):
    """予約可能なホテル名・年月日・空室数をテキストにして返す."""
    text = f"{last_reservation_date} \n"
...
def get_reservable_info_as_text():
    ...
    # テキストに整形
    reservable_info_as_txt = convert_reservable_info_into_text(
        last_reservation_date, reservable_info
    )
